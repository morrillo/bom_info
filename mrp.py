...
class mrp_bom(osv.osv):
    ...
    def _calc_delay(self, cr, uid, ids, context=None):
	...
	bom_lines = bom.child_complete_ids
       	if bom_lines:
            for sbom in bom_lines:
                delay += self._calc_delay(cr, uid, sbom.id) 
	else:
	    	bom_obj = self.pool.get('mrp.bom')
        	no_child_bom = bom_obj.search(cr, uid, [('product_id', '=', bom.product_id.id), ('bom_id', '=', False)])
       	        if no_child_bom and bom.id not in no_child_bom:
	            other_bom = bom_obj.browse(cr, uid, no_child_bom)[0]
        	    delay += other_bom.product_id.produce_delay
	        else:
		    delay += bom.product_id.produce_delay
  	res = delay
	return res

    def _calc_real_delay(self, cr, uid, ids, context=None):

	bom = self.pool.get('mrp.bom').browse(cr,uid,ids,context=context)
	if bom.product_id.virtual_available > 0:
		res = 0
	else:
	        delay = 0
		bom_lines = bom.child_complete_ids
    ...
    def _fnct_calc_delay(self, cr, uid, ids, field_name, args, context=None):
	""" Calculates theoretical delay for the product, not considering the available stock """
	res = {}
	for bom in self.browse(cr,uid,ids):
		delay = self._calc_delay(cr,uid,bom.id)
		res[bom.id] = delay
	return res

    def _fnct_calc_real_delay(self, cr, uid, ids, field_name, args, context=None):
	""" Calculates real delay for the product, considering the available stock """
	res = {}
	for bom in self.browse(cr,uid,ids):
		delay = self._calc_real_delay(cr,uid,bom.id)
		res[bom.id] = delay
	return res


    def _fnct_calc_price(self, cr, uid, ids, field_name, args, context=None):
	""" Calculates product cost, considering the cost of the products included in the BoM """
	...
	'calculate_price': fields.boolean('Calculate Price'),
        ...
        'theoretical_manuf_delay': fields.function(_fnct_calc_delay, string='Theoretical Manufacturing Delay'),
        'real_manuf_delay': fields.function(_fnct_calc_real_delay, string='Actual Manufacturing Delay'),
